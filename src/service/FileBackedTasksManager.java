...
//1,2,3
    ...
    private final static Path FILE = Paths.get("./kanban.csv");
    ...
    static FileBackedTasksManager loadFromFile() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(String.valueOf(FILE), StandardCharsets.UTF_8))) {
            ...
            for (int i = 1; i < taskLines.size() - 2; i++) { //обходим каждую задачу
                String[] line = taskLines.get(i).split(",");
                Task task = fromString(line);//получаем задачу
                history.put(task.getId(), task); //складываем в мапу для истории
            ...
            List <Integer> ids = historyFromString(taskLines.get(taskLines.size() - 1));
            for (Integer id : ids) {
                historyManager.add(history.get(id));
                //где historyManager.getHistory() - список истории, (fileManager.tasks.get(id)) - задача
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
